---
layout: post
title:  uv Monorepo Workspaces
categories: [Python, Packaging, uv]
excerpt: Managing a complex Python repository with a shared core, various plugins, and multiple applications used to be a headache involving brittle path dependencies or complex scripts. In 2025, the uv workspace has emerged as the definitive solution for these "monorepo" architectures.
...
## What is a Workspace?
uv's workspace concept is [heavily inspired by Rust's Cargo](https://docs.astral.sh/uv/concepts/workspaces/) -- a high performance tool for managing large codebases with multiple related packages. The most important concepts to know are:
...
* **Workspace root:** Workspace members are defined in the `[tool.uv.workspace]` table of the workspace root `pyproject.toml` file. This root is also considered a workspace member.
...
* `uv sync` from the root: creates a virtual environment, install dependencies, build and install workspace packages.
  ...
  Note: You can run `uv sync --all-packages` from a workspace member directory to achieve the same effect as running `uv sync` from the workspace root.

* Isolated Testing: To run tests for just one plugin without other workspace noise:
  ```bash
  # from the workspace root
  uv sync --package my-plugin --group test
  uv run --package my-plugin --directory packages/my-plugin/ pytest tests
  ```

  You could also navigate into a workspace member's directory:
  ```bash
  ...
  Note: make sure pytest is installed when you run `uv run pytest`. If pytest is not installed as a direct dependency of the package you are in, uv falls back to a system-wide pytest or one from a different environment that doesn't "know" about your local workspace members (see [below](#uv-appears-to-use-a-different-unexpected-virtual-environment) for a deeper dive).
...
* Test Individual Packages (Isolated): To ensure a package isn't accidentally relying on "ghost dependencies" from its siblings, sync specifically to that package before testing.
  ...
  uv run --package my_pkg --directory packages/my_pkg/ pytest tests
  ```

  Alternatively, test within a workspace member's directory
  ```bash
  cd packages/my-plugin
  uv sync --group test
  uv run pytest tests
...
For more details, see the [official uv workspace documentation](https://docs.astral.sh/uv/concepts/workspaces/).
